...
from quiz.models import Quiz,Title,UserScore,Language,Feedback
...
def startQuiz(request):
    ...
    for lang in languages:
        user_play = []
        noOfQues = []
        titles = Title.objects.filter(Language=lang)[:4]
        for title in titles:
...
def handleQuiz(request):
    if request.method == 'POST':
        ...
        if time_remaining == "-1:-1" or time_remaining == "00:00":
...
def category(request,slug):
    user_play = []
    noOfQues = []
    title = Title.objects.filter(Language=slug)
    for tit in title:
    ...
    if title.count() == 0:
        return HttpResponse("404 Page not found")
    return render(request,'quiz/quizCategory.html',{"titles":alltitle})

def viewTitleScore(request,slug):
    if request.user.is_authenticated:
        userScore = UserScore.objects.filter(quizUsername=request.user,quizTitle=slug).order_by('-played_date')
        if userScore.count() != 0:
            context = {"userscore":userScore}
        else:
            context = {"notPlay":True}
